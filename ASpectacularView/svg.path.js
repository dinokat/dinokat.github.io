...
(function() {
	...
	SVG.extend(SVG.Path, {
    getString: function(){
      return this._getString(this._segments);

    },
    setShape: function(pts){
      var b = this._redrawEnabled;
      if (pts.length == 0){
        return this;
      }
      else{
        ...
        this.clearPath();
        this.M(pts[0].x, pts[0].y);
        for (var i =1; i<pts.length;i++){
          this.L(pts[i].x, pts[i].y);
        }
        this._redrawEnabled = b;
        this.redraw()
      }
      return this;//.plot();
