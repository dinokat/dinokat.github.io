...
(function() {
  ...
  Bezier.prototype = {
    ...
    toSVGjoin: function(svgstr) {
      ...
      return svgstr.concat(s.join(" "));
    },

    multyplyTension: function(dk) {
      if(this._3d) return false;
      var p = this.points,
          x1 = p[0].x,
          y1 = p[0].y,
      ...
      p[1].x = (p[1].x - x1) * dk + x1;
      p[1].y = (p[1].y - y1) * dk + y1;
      p[2].x = (p[2].x + x2) * dk + x2;
      p[2].y = (p[2].y + y2) * dk + y2;
      return true;
    },
...
},{"./poly-bezier.js":2,"./utils.js":3}],2:[function(require,module,exports){
(function() {
  "use strict";
  alert("DEFING POLYBEZIER!!");
  var utils = require('./utils.js');
  ...
  PolyBezier.prototype = {
    ...
    ZtoSVG: function() {
      str = this.curves[0].toSVG();
      for(var i =1; i<this.curves.length;i++){
        str=this.curves[i].toSVGjoin(str);
      }
      return str;
    },
...
(function() {
  ...
  var utils = {
    ...
    ZpointsToString: function(points) {
      return "[" + points.map(utils.ZpointToString).join(", ") + "]";
    },
