...
},{"./poly-bezier.js":2,"./utils.js":4}],2:[function(require,module,exports){
(function() {
  "use strict";
...
(function() {
  ...
  */
  Punt = class Punt {
    ...
    mirrorX(x) {
      return this.x += 2 * (x - this.x);
    }

  };

  window.Punt = function(x, y) {
    return new Punt(x, y);
  };

  window.solvecheck = function(data, k) {
    var BZ, bz, bzz, cp1x, cp1y, cp2x, cp2y, i, last, pbezier, size, x0, x1, x2, x3, y0, y1, y2, y3;
    if (k === null) {
      k = 1;
    }
    size = data.length;
    ...
    i = 0;
    while (i < size - 2) {
      x0 = i ? data[i - 2] : data[size - 2];
      y0 = i ? data[i - 1] : data[size - 1];
      x1 = data[i + 0];
      y1 = data[i + 1];
      x2 = data[i + 2];
      y2 = data[i + 3];
      x3 = i !== last ? data[i + 4] : data[0];
      y3 = i !== last ? data[i + 5] : data[1];
      cp1x = x1 + (x2 - x0) / 6 * k;
      cp1y = y1 + (y2 - y0) / 6 * k;
      cp2x = x2 - ((x3 - x1) / 6 * k);
      cp2y = y2 - ((y3 - y1) / 6 * k);
      ...
      if ((function() {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          BZ = ref[j];
          results.push(bz.intersects(BZ).length);
        }
        return results;
      })()) {
        return null;
   ...
   */
  window.MAAK2 = function(h, c, r, tens) {
    var TMP, d, del, parray, pi2;
    TMP = [];
...
},{"./bezier.js":1}],4:[function(require,module,exports){
(function() {
  "use strict";
